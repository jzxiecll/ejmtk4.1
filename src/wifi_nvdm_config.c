...
int32_t user_wifi_config_init(wifi_cfg_t *wifi_config)
{
#ifdef MTK_WIFI_PROFILE_ENABLE
...
{
    int status = 0;
	printf("save_cf_credential_to_nvdm***********************\r\n");
    if (cred == NULL) {
...
int32_t get_smart_conf_to_nvdm(uint8_t *config_ready)
{
    char buf[WIFI_PROFILE_BUFFER_LENGTH] = {0};
    uint32_t len = sizeof(buf);
    ...
    if (NVDM_STATUS_OK != nvdm_read_data_item(WIFI_PROFILE_BUFFER_COMMON, "Smartconfig_Ready", (uint8_t *)buf, &len)) {
        return WIFI_FAIL;
    }
...
int32_t set_smart_conf_to_nvdm(uint8_t config_ready)
{
    ...
    memset(buf, 0x0, WIFI_PROFILE_BUFFER_LENGTH);
    sprintf(buf, "%d", config_ready);
    LOG_I(wifi, "Smartconfig_Ready : %s", buf);

    if (NVDM_STATUS_OK != nvdm_write_data_item(WIFI_PROFILE_BUFFER_COMMON, "Smartconfig_Ready",
            NVDM_DATA_ITEM_TYPE_STRING,
            (uint8_t *)buf, strlen(buf))) {
        return WIFI_FAIL;
    }
...
int save_smt_result_to_nvdm(uint8_t *Ssid,uint8_t *WpaPsk)
{
    int status = 0;
	printf("save_smt_result_to_nvdm***********************\r\n");
    if (cf_set_ssid(WIFI_PORT_STA, Ssid, strlen(Ssid)) != 0) {
        LOG_E(wifi, "ERROR! [ConfigFree][STA] wifi_profile_set_ssid failed (Ssid=%s, Len=%d)",Ssid, strlen(Ssid));
        status = -1;
    }
    if (cf_set_wpa_psk_key(WIFI_PORT_STA, WpaPsk, strlen(WpaPsk)) != 0) {
        LOG_E(wifi, "ERROR! [ConfigFree][STA] wifi_profile_set_wpa_psk_key failed (WpaPsk=%s, Len=%d)", WpaPsk, strlen(WpaPsk));
        status = -1;
    }
